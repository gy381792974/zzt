    ...
    public class JsonReader
    {
        ...
        private void ProcessNumber (string number)
        {
                ...
                number.IndexOf('E') != -1)
            {
                ...
                if (splitArray.Length > 1)
                {
                   // 精度6--认为是float
                    if (splitArray[1].Length < 7)
                    {
                        ...
                        token_value = float.Parse(number);
                        return;
                    }
                    //否则认为是double
                    token = JsonToken.Double;
                    token_value = double.Parse(number);
                    return;
                }
                ...
                if (double.TryParse(number, NumberStyles.Number, CultureInfo.InvariantCulture, out doubleValue))
                {
                    token = JsonToken.Double;
                    token_value = doubleValue;
                    return;
                }

                if (BigInteger.TryParse(number, NumberStyles.Any, CultureInfo.InvariantCulture, out bigIntegerValue))
                {
                    token = JsonToken.BigInter;
                    token_value = bigIntegerValue;
                    return;
                }
            }
            ...
            if (BigInteger.TryParse(number, NumberStyles.Any, CultureInfo.InvariantCulture, out bigInteger))
            {
                token = JsonToken.BigInter;
                token_value = bigInteger;
