...
namespace UnityEngine.UI
{
    ...
    public class ScrollViewInfinity : UIBehaviour, IInitializePotentialDragHandler, IBeginDragHandler, IEndDragHandler, IDragHandler, IScrollHandler, ICanvasElement, ILayoutGroup
    {
        #region 定义结构
        ...
        protected override void Awake()
        {
            ...
            itemSize = new Vector2(tf.rect.width, tf.rect.height);
            this.fixedCount = 1;
        }

        public void InitScrollView(int dataNum, int fixedCount = 1)
        {
            numItems = (uint)dataNum;
            this.fixedCount = (uint)fixedCount;
            //RectTransform tf = prefab.GetComponent<RectTransform>();
            //itemSize = new Vector2(tf.rect.width, tf.rect.height);
            //this.fixedCount = (uint)fixedCount;
        }
